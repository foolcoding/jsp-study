...
<h1>페이지 이동 기술</h1>
<hr>
<h3>redirect : 클라이언트에게 이동할 주소를 보내주고, 클아이언트가 새로 요청하게 한다</h3>
...
<table border="1" cellpadding="10" cellspacing="0">
	<tr>
		...
		<td>
			<ul>
				<li>클라이언트가 새로운 요청을 보내기 때문에, 현재 서버가 아닌 다른 주소도 이용 가능하다</li>
				<li>클라이언트가 직접 요청을 보내기 때문에, 브라우저 주소창의 주소가 변경된다</li>
				<li>새로 연결할 전화번호만 알려주고 현재 통화를 끊는 형식</li>
				<li>request, response 새로 만들어진다, 301 302</li>
			</ul>
		</td>
	</tr>
	<tr>
		<td>response.getRequestDispatcher(String url).forward(request, response)</td>
		<td>
			<ul>
				<li>서버 내부에서 임의로 응답할 주소를 고르기 때문에, 다른 서버로는 넘길 수 없다</li>
				<li>클라이언트는 관여하지 않고 서버에서 응답을 바꿔치기하므로, 브라우저 주소창이 변경되지 않는다</li>
				<li>고객에게 잠시 기다리게 하고, 내선번호로 전화를 올리는 형식</li>
...
<fieldset>
	...
	<form>
		<input type="text" name="redirectURL" placeholder="리다이렉트 주소">
		<button>리다이렉트</button>
	</form>
</fieldset>
...
<fieldset>
	...
	<form>
		<input type="text" name="forwardURI" placeholder="포워드 경로 주소">
		<button>포워드</button>
	</form>
</fieldset>
