...
<h1>request response 내장 객체</h1>
<hr>

<h3>스크립틀릿 내부에서는 객체를 별도로 생성하지 않아도 request/response에 접근할 수 있다</h3>
<h3>request 와 response는 JSP의 내장객체에 해당한다 (별도의 생성이 필요없다)</h3>

<table>
	<thead>
		<tr>
			<th colspan="3">response 주요 메서드</th>
		</tr>
		<tr>
			<td>형식</td>
			<td>기능</td>
	...
	<tbody>
		...
		<tr>
			...
			<td>요청 프로토콜을 반환, http 혹은 https</td>
			<td><%= request.getProtocol() %></td>
		</tr>
		<tr>
			<td>request.getServerName()</td>
			<td>요청 서버 주소를 반환, 글자 주소를 우선으로 제공, 없으면 IP</td>
			<td><%= request.getServerName() %></td>
		</tr>
		<tr>
			<td>request.getServerPort()</td>
			<td>요청 서버의 포트 번호를 반환, 범위는 0~65535</td>
			<td><%= request.getServerPort() %></td>
		</tr>
		<tr>
			<td>request.getContextPath()</td>
			<td>
				요청받은 자원의 최상위 폴더경로를 문자열로 반환<br>
				이 경로는 JSP 프로젝트의 webapp(WebContent)경로에 해당
			</td>
			<td><%= request.getContextPath() %></td>
		</tr>
		<tr>
			<td>request.getRequestURI()</td>
			<td>요청받은 자원 교유 식별자를 문자열로 반환, 전체주소가 아님</td>
			<td><%= request.getRequestURI() %></td>
		</tr>
		<tr>
			<td>request.getParameter("guest")</td>
			<td>
				요청에 담긴 클라이언트의 파라미터를 문자열로 반환
		...
		<tr>
			...
			<td>주소에서 URL을 제외한 쿼리스트링을 문자열로 반환</td>
			<td><%= request.getQueryString() %></td>
		</tr>
	</tbody>
</table>
