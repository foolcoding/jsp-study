...
<table>
	<thead>
		...
		<tr>
			...
			<td>JSP가 클래스로 변환될 때 맴버 필드 및 맴버 메서드에 해당하는 항목을 작성한다</td>
			<td>&lt;%! private String str = "Hello, world !!"" %></td>
		</tr>
		<tr>
			<td>스크립틀릿, scriptlet</td>
			<td>JSP가 클래스로 변환될 때 service() 메서드 내부 내용을 작성한다 (main과 유사함)</td>
			<td>&lt;% list.add("apple"); %></td>
		</tr>
		<tr>
			<td>표현식, expression</td>
			<td>HTML 태그 중간에 Java객체와 값을 출력하기 위해 사용한다 (out.print() 와 동일함)</td>
			<td>&lt;%= str %></td>
		</tr>
	</thead>
</table>
